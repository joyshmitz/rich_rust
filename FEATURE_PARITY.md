...
| Traceback | Traceback rendering + `Console::print_exception` | Partial | `src/renderables/traceback.rs` | Deterministic synthetic frames + Python fixture conformance (`traceback/basic`). Code context via `extra_lines` + `source_context` (from file or embedded). No locals or automatic Rust backtrace capture yet. |
